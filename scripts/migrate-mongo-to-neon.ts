...
if (!sourceUri) {
	throw new Error('DATABASE_URI or SOURCE_DATABASE_URI is required for the legacy-data migration');
}
...
function dateOf(value: unknown, fallback?: Date): Date {
	...
	throw new Error(`Expected a valid date, received ${JSON.stringify(value)}`);
}
	...
	for (const collectionName of names) {
		...
		try {
			for await (const document of cursor) {
				const sourceId = idOf(document._id) || '<missing-id>';
				try {
					await recordNullByteTransform(collectionName, document);
					await handler(collectionName, document);
				} catch (error) {
					throw new Error(`Failed to migrate ${collectionName}/${sourceId}`, { cause: error });
				}
...
async function migrateAuth(collectionName: string, document: RawDocument): Promise<void> {
	...
	if (collectionName === 'rateLimit') {
		const key = String(document.key ?? id);
		await upsert(
			'auth.rate_limits',
			['id', 'key', 'count', 'last_request'],
			[id, key, intValue(document.count), intValue(document.lastRequest)],
			'"key"',
			['count', 'last_request']
		);
		await recordLedger(collectionName, id, 'auth.rate_limits', key, document);
		return;
	}
...
async function migrateSuper(collectionName: string, document: RawDocument): Promise<void> {
	...
	const targetTable =
		collectionName === 'super_cleanup_jobs' ? 'ops.super_cleanup_jobs' : `app.${collectionName}`;
	await recordLedger(collectionName, sourceId, targetTable, sourceId, document);
}
...
async function migrateQuality(collectionName: string, document: RawDocument): Promise<void> {
	...
	const targetTable = {
		...
		question_quality_feedback: 'content.question_feedback',
		question_quality_review_jobs: 'content.quality_review_jobs',
		question_quality_review_job_items: 'content.quality_review_job_items'
	}[collectionName];
	if (!targetTable) throw new Error(`No quality target table for ${collectionName}`);
	await recordLedger(collectionName, sourceId, targetTable, sourceId, document);
}
...
async function migrateOperations(collectionName: string, document: RawDocument): Promise<void> {
	...
	const targetTable = {
		poolrefillstates: 'ops.pool_refill_states',
		poolbucketwritelocks: 'ops.pool_bucket_write_locks',
		poolgenerationbudgets: 'ops.pool_generation_budgets'
	}[collectionName];
	if (!targetTable) throw new Error(`No operations target table for ${collectionName}`);
	await recordLedger(collectionName, sourceId, targetTable, sourceId, document);
}
...
async function load(db: Db): Promise<void> {
	...
	await eachDocument(db, ['blogposts', 'cachemisslocks'], archiveLegacyDocument);
	...
	await eachDocument(db, [...frqPoolNames], (collection, document) =>
...
async function verify(db: Db): Promise<void> {
	...
	const ledgerRows = (await query(
		'SELECT source_collection, source_id, checksum FROM ops.migration_ledger WHERE run_id = $1',
		[runId]
	)) as Array<{ source_collection: string; source_id: string; checksum: string }>;
	const ledgerChecksums = new Map<string, string>();
	for (const row of ledgerRows) {
		const ledgerKey = `${row.source_collection}\u001f${row.source_id}`;
		if (ledgerChecksums.has(ledgerKey)) {
			throw new Error(
				`Migration ledger has duplicate rows for ${row.source_collection}/${row.source_id}`
			);
		}
		ledgerChecksums.set(ledgerKey, row.checksum);
	}
	for (const collectionName of collections) {
		if (
			...
			continue;
		let count = 0;
		const cursor = db.collection<RawDocument>(collectionName).find({}).batchSize(batchSize);
		for await (const document of cursor) {
			count += 1;
			const documentId = requiredId(document, collectionName);
			const sourceIds =
				collectionName === 'betterAuthMigrationMap' && document.legacyUserId
					? [String(document.legacyUserId), documentId]
					: [documentId];
			const sourceId = sourceIds.find((candidate) =>
				ledgerChecksums.has(`${collectionName}\u001f${candidate}`)
			);
			if (!sourceId) {
				throw new Error(`Migration ledger is missing ${collectionName}/${documentId}`);
			}
			const ledgerKey = `${collectionName}\u001f${sourceId}`;
			const migratedChecksum = ledgerChecksums.get(ledgerKey);
			const sourceChecksum = checksum(document);
			if (migratedChecksum !== sourceChecksum) {
				throw new Error(`Migration checksum mismatch for ${collectionName}/${sourceId}`);
			}
			ledgerChecksums.delete(ledgerKey);
		}
		console.log(
			JSON.stringify({ phase: 'verify-source-count', collection: collectionName, count })
		);
	}
	if (ledgerChecksums.size) {
		throw new Error(`Migration ledger has ${ledgerChecksums.size} rows without source documents`);
	}
