...
export interface BotSampleOptions {
	/** Fewer points are visually sufficient for the small chat avatars. */
	arcSamples?: number;
}
...
export class BotEngine {
	...
	private cachedIdleBodyPath(sil: Silhouette): string {
		const cached = this.idleBodyPath;
		if (
			cached &&
			cached.radii === sil.radii &&
			cached.rot === sil.rot &&
			cached.cx === sil.cx &&
			cached.cy === sil.cy &&
			cached.sx === sil.sx &&
		...
		const path = closedPath(toPoints(sil, this.scale, this.pts));
		this.idleBodyPath = { ...sil, path };
		return path;
	}
	...
	sample(now: number, options: BotSampleOptions = {}): BotFrame {
		const R = this.scale;
		const def = STATE_BY_ID.get(this.cur)!;
		const shape = this.shapeAtTime(now);
		const expr = this.exprAtTime(now);
		...
		// --- corps ------------------------------------------------------------
		const stableIdle = this.cur === 'idle' && since >= def.morph && shape === this.shape;
		const sil: Silhouette = stableIdle
			? pose.sil
			: {
					...pose.sil,
					cx: pose.sil.cx + offX,
		...
		const bodyPath = stableIdle
			? this.cachedIdleBodyPath(sil)
			: closedPath(toPoints(sil, R, this.pts));
		...
		return {
			...
			arcs: pose.arcs
				...
				.map((a) => arcRender(a.seed, a.t, R, a.id, a.opacity, options.arcSamples)),
			notif,
			notch
		};
	}
