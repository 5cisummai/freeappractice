...
function isAgentCalibrated(): boolean {
	return (
...
async function updateQualityFromBatchLine(
	...
	try {
		...
		const assessment = parseAssessmentText(extractResponseOutputText(body), {
			...
			inputTokens,
			outputTokens,
			estimatedCostUsd: actualCost(inputTokens, outputTokens),
			...webEvidence
		});
...
function toJobSummary(job: Partial<ReviewJobDocument> & { _id?: unknown }): QualityJobSummary {
	return {
		id: String(job._id),
