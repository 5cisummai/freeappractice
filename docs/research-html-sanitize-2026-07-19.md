...
4. **If production is crashing today and F is not ready:** temporary **A** (`overrides: { "jsdom": "25.0.1" }`) *or* **A′** (`NODE_OPTIONS=--experimental-require-module`), then remove the workaround when F lands.
