...
The application database is Neon Postgres reached through `@neondatabase/serverless` and Drizzle's `neon-http` adapter. `NEON_DATABASE_URL` is the normal Neon `postgresql://...` connection string; the selected driver sends it over HTTPS. Runtime code must not import `pg`, `postgres`, or the WebSocket driver.
